# Adapted from inference/caption_generator.py
# Changes:
#   - Beam imported from modules.beam_t5 (T5 BOS/EOS tokens)
#   - decode_tokens_to_text imported from inference.eval_utils_t5 (T5 decode)
#   - output_caption_ids decoded with t5_tokenizer (skip_special_tokens)
# Everything else (beam search logic, collate, eval loop) is identical.

import torch
from modules.beam_t5 import Beam
from inference.eval_utils_t5 import decode_tokens_to_text, save_predictions, save_complete_results, log_metrics
...
def beam_decode_step(decoder, inst_dec_beams, len_dec_seq,
    ...
    active_inst_idx_list = collect_active_inst_idx_list(inst_dec_beams, word_prob,
...
def eval_epoch(args, model, test_dataloader, t5_tokenizer, device, n_gpu, logger, nlgEvalObj=None, test_set=None):
    """
    Evaluate captioning with beam search.

    Args:
        t5_tokenizer: T5Tokenizer instance used for decoding generated token ids.
    """
    ...
    for batch in test_dataloader:
        ...
        with torch.no_grad():
            ...
            if isinstance(loss, dict):
                loss = loss.get("decoder_loss", loss.get("loss", loss))
            if n_gpu > 1:
                loss = loss.mean()
            ...
            for re_list in result_list:
                # decode_tokens_to_text uses T5Tokenizer.decode (eval_utils_t5)
                decode_text = decode_tokens_to_text(re_list, t5_tokenizer)
                all_result_lists.append(decode_text)

            for re_list in caption_list:
                # output_caption_ids contain T5 token ids; -1 positions are padding (ignore_index)
                clean_ids = [int(x) for x in re_list if int(x) >= 0]
                decode_text = decode_tokens_to_text(clean_ids, t5_tokenizer)
    ...
    # File writes only on rank 0 to avoid concurrent write conflicts in multi-GPU
